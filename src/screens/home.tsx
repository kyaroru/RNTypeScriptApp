...
const Home: FC<HomeScreenProps> = props => {
  const [homeState, setHomeState] = useState<HomeScreenState>({
    selectedCategory: 0,
    images: [
  ...
  const onLayout = (e: { nativeEvent: { layout: { width: any; }; }; }) => {
    setHomeState((originalHomeState) => ({
      ...originalHomeState,
  ...
  const renderCategories = () => {
    const { categories, fetchProducts, isLoadingCategories } = props;
    const { selectedCategory } = homeState;
    return (
      <CategoryPanel
  ...
  const renderContent = () => {
    const { products, isLoadingProducts, navigation } = props;
    const { selectedCategory, icons } = homeState;
    return (
